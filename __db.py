...
class SiteTable:
    ...
    @staticmethod
    def drop(conn: sqlite3.Connection):
        q = f"DROP TABLE IF EXISTS {SiteTable.table}"
        execute(conn, q)

    @staticmethod
    def create(conn: sqlite3.Connection):
        sql = f"""
        CREATE TABLE IF NOT EXISTS {SiteTable.table} (
            id INTEGER NOT NULL, 
            uri TEXT NOT NULL, 
    ...
    @staticmethod
    def insert(conn: sqlite3.Connection, vals_list=[]):
        sql = f"INSERT OR IGNORE INTO {SiteTable.table} (uri, example) VALUES (?, ?)"
        executemany(conn, sql, vals_list)
    ...
    @staticmethod
    def select_all(conn: sqlite3.Connection):
        return execute(conn, f"SELECT * FROM {SiteTable.table}").fetchall()
...
class StepTable:
    ...
    @staticmethod
    def drop(conn: sqlite3.Connection):
        q = f"DROP TABLE IF EXISTS {StepTable.table}"
        execute(conn, q)

    @staticmethod
    def create(conn: sqlite3.Connection):
        sql = f"""
        CREATE TABLE IF NOT EXISTS {StepTable.table} (
    ...
    @staticmethod
    def insert(conn: sqlite3.Connection, vals_list=[]):
        sql = f"INSERT OR IGNORE INTO {StepTable.table} (site_id, step, selector, action, value) VALUES (?, ?, ?, ?, ?)"
        executemany(conn, sql, vals_list)
...
class AdTable:
    ...
    @staticmethod
    def drop(conn: sqlite3.Connection):
        q = f"DROP TABLE IF EXISTS {AdTable.table}"
        execute(conn, q)

    @staticmethod
    def create(conn: sqlite3.Connection):
        execute(conn, AdTable.sql_create)

    @staticmethod
    def insert(conn: sqlite3.Connection, vals_list=[]):
        sql = f"""
        INSERT OR IGNORE INTO {AdTable.table} 
